...
namespace assignment
{
    public partial class Form1 : Form
    {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            //최소 밝기값,중간,최대 밝기값을구한다.
            //즉 삼각소속도의 오른쪽,왼쪽 끝점과 중간점을찾는부분.
            Imax = Xmean + a; //최대
            Imin = Xmean - a;//최소
            Imid = (Imax + Imin) / 2; //중간

            //소속도를 구하고 alpha 와 beta를 구한다.
            //percent = cut을 의미한다.
            if (Imin != 0) percent = (double)Imin / (double)Imax;
            I_value = (Imid - Imin) * percent + Imin; //alpha
            r_value = -(Imax - Imid) * percent + Imax; //beta
            ...
            for (x = 255; x > beta; x--) LUT[x] = 255; //beta 보다클때까지 255로준다
            for (x = alpha; x <= beta; x++)//alpha 와 beta사이를 스트레칭 공식으로 해준다.
            {
